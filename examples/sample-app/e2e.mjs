...
if (!res.ok) {
  console.error(`x expected 2xx, got ${res.status}`);
  process.exit(1);
}
// Proves the whole chain: proxy -> web -> api.internal -> db
if (body.api?.db !== true) {
  console.error("x api could not reach the database");
  process.exit(1);
}
if (body.worktree !== process.env.WT_NAME) {
  console.error(`x served by worktree "${body.worktree}", expected "${process.env.WT_NAME}"`);
  process.exit(1);
}

console.log(`ok e2e passed - proxy -> web -> api -> db, all inside worktree "${body.worktree}"`);
